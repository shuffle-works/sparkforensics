...
Advanced view now has single-key triage shortcuts: `j`/`k` move between the verdict's steps and
every finding row, `Enter` opens the focused finding's evidence, `f` jumps to the finding filters,
