---
"sparkforensics-web": minor
---

The dashboard now opens with a run verdict: one line saying where to start, a short summary, and
up to three numbered next steps. Each step says in plain language what is happening and what to
try, and links to its evidence and stage details. Findings on the same stage fold into one step,
since their savings overlap. When most of the run's executor capacity sat idle, the verdict leads
with cluster size instead of a small per-stage fix. The separate "Highest impact" callout is gone,
and the finding filter bar now shows in Advanced view, or whenever a filter is active.
