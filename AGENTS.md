...
Tuning reference: `npm run docs:bump [-- <sha>]` moves the pinned `shuffle-works/spark-tuning-reference` commit in `packages/core/src/docs-content/upstream.json` (default: upstream head), regenerates `docs-content/{chapters,tuning,diagrams}` and writes the changeset; put the compare link it prints in the PR body. `npm run docs:fetch` regenerates from the current pin, `-- --check` (run in CI) fails unless the pin reproduces the committed copy byte for byte. `npm run docs:bump -- --check [<sha>]` runs the anchor gate against upstream head (or `<sha>`) and writes nothing; `.github/workflows/tuning-reference-drift.yml` runs it plus `test:core` weekly. Never hand-edit those three dirs: fix upstream and bump.
