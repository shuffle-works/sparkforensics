...
Tuning reference: `npm run docs:bump [-- <sha>]` moves the pinned `shuffle-works/spark-tuning-reference` commit in `packages/core/src/docs-content/upstream.json` (default: upstream head), regenerates `docs-content/{chapters,tuning,diagrams}` and writes the changeset; put the compare link it prints in the PR body. `npm run docs:fetch` regenerates from the current pin, `-- --check` (run in CI) fails unless the pin reproduces the committed copy byte for byte. Never hand-edit those three dirs: fix upstream and bump.
