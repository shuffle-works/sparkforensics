// tests/view/over-broadcast.test.tsx
...
import { OverBroadcast } from '@/view/widgets/OverBroadcast';
...
function finding(overrides: Record<string, unknown> = {}): Finding {
  return {
    ...
    value: 900 * 1024 * 1024,
    recommendation: 'Lower spark.sql.autoBroadcastJoinThreshold or remove the broadcast hint.',
    ...overrides,
  ...
  return render(
    <DocsProvider>
      <EvidenceAvailabilityProvider>
        <OverBroadcast catalog={catalog} defaultCollapsed={defaultCollapsed} />
      </EvidenceAvailabilityProvider>
    </DocsProvider>,
  );
}

test('renders nothing when no overBroadcast finding is present', () => {
...
test('renders a WidgetCard heading with the PLAN tag', async () => {
  ...
  expect(screen.getByRole('heading', { name: 'Oversized Broadcast Join' })).toBeInTheDocument();
...
test('flags every finding, each with its own recommendation and stage pills', () => {
  renderWidget([
    finding({ stageIds: [1], recommendation: 'over rec A' }),
    finding({ stageIds: [2], recommendation: 'over rec B' }),
  ]);
  expect(screen.getByText(/over rec A/)).toBeInTheDocument();
  expect(screen.getByText(/over rec B/)).toBeInTheDocument();
...
test('defaults to impact order (highest potential savings first), and a toggle flips it back at advanced density', async () => {
  ...
  store.getState().setWidgetDensity('advanced');
  renderWidget([
    finding({ stageIds: [1], recommendation: 'Shrink stage 1', impactEstimate: { basis: 'serial', wallClock: { low: 100, high: 100 }, estimateMethod: 'measured' } }),
    finding({ stageIds: [2], recommendation: 'Shrink stage 2', impactEstimate: { basis: 'serial', wallClock: { low: 5000, high: 5000 }, estimateMethod: 'measured' } }),
  ]);

  const listItems = () => screen.getAllByRole('listitem').map((li) => li.textContent ?? '');
  await expectImpactThenStageOrderByText(user, listItems, /Shrink stage 1/, /Shrink stage 2/);
  store.getState().setWidgetDensity('basic');
});
...
test('paginates 6-at-a-time with Previous/Next controls', async () => {
  const user = userEvent.setup();
  const findings = Array.from({ length: 7 }, (_, i) => finding({ stageIds: [i], recommendation: `Shrink stage ${i}` }));
...
test('card defaults collapsed and shows a finding-count summary', async () => {
  ...
  // Lead value is the distinct stage count, not a repeat of the "Oversized Broadcast Join" title.
  expect(screen.getByText('2 stages')).toBeInTheDocument();
  expect(screen.getByText('2 findings flagged')).toBeInTheDocument();
  await user.click(screen.getByRole('button', { name: 'Oversized Broadcast Join' }));
