...
import { DuplicatePlanSubtree } from '@/view/widgets/DuplicatePlanSubtree';
...
function finding(overrides: Record<string, unknown> = {}): Finding {
  return {
    ...
    stageIds: [5, 6],
    impactBand: 'warning',
    metric: 'subtreeOccurrences',
    value: 2,
    rootName: 'SortMergeJoin',
    subtreeSize: 3,
  ...
  return render(
    <DocsProvider>
      <EvidenceAvailabilityProvider>
        <DuplicatePlanSubtree catalog={catalog} defaultCollapsed={defaultCollapsed} />
      </EvidenceAvailabilityProvider>
    </DocsProvider>,
  );
}

test('renders nothing when no duplicatePlanSubtree finding is present', () => {
...
test('renders a WidgetCard heading with the PLAN tag', async () => {
  ...
  expect(screen.getByRole('heading', { name: 'Redundant Plan Subtree' })).toBeInTheDocument();
...
test('omits the header-level confidence badge when findings in the group disagree on confidence', () => {
  store.getState().setWidgetDensity('advanced');
  renderWidget([
    finding({ stageIds: [1], confidence: 'medium', validationRequired: 'verify me' }),
    finding({ stageIds: [2], confidence: 'high' }),
  ]);
...
test('paginates 6-at-a-time with Previous/Next controls', async () => {
  const user = userEvent.setup();
  const findings = Array.from({ length: 7 }, (_, i) => finding({ stageIds: [i], recommendation: `Dedupe stage ${i}` }));
